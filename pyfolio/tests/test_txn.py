...
class TransactionsTestCase(TestCase):
    ...
    def test_get_turnover(self):
        ...
        positions = pd.DataFrame([[10.0, 10.0]] * len(dates),
                                 columns=[0, 'cash'],
                                 index=dates)
        ...
        transactions = pd.DataFrame(data=[[1, 1, 10, 0]] * len(dates) +
                                         [[2, -1, 10, 0]] * len(dates),
                                    columns=['sid', 'amount', 'price', 'symbol'],
    ...
    def test_adjust_returns_for_slippage(self):
        ...
        positions = pd.DataFrame([[0.0, 10.0]] * len(dates),
                                 columns=[0, 'cash'], index=dates)

        # 100% total, 50% average daily turnover
        transactions = pd.DataFrame(data=[[1, 1, 10, 'A']] * len(dates),
                                    columns=['sid', 'amount', 'price', 'symbol'],
                                    index=dates)
