...
def plot_rolling_risk_factors(my_algo_returns, risk_factors, rolling_beta_window=63, legend_loc='lower left', end_date=None):
    ...
    plt.title("Rolling Fama-French Single Factor Betas (" +  num_months_str + '-month)',fontsize=16)
    plt.ylabel('beta',fontsize=14)

    rolling_risk_multifactor = timeseries.rolling_multifactor_beta(my_algo_returns, risk_factors.ix[:,['SMB','HML','UMD']], 
                                                            rolling_window=rolling_beta_window)

    rolling_beta_SMB = timeseries.rolling_beta(my_algo_returns, risk_factors['SMB'], rolling_window=rolling_beta_window)
    rolling_beta_HML = timeseries.rolling_beta(my_algo_returns, risk_factors['HML'], rolling_window=rolling_beta_window)
    rolling_beta_UMD = timeseries.rolling_beta(my_algo_returns, risk_factors['UMD'], rolling_window=rolling_beta_window)
    ...
    (rolling_risk_multifactor['const']*252).plot(color='forestgreen', alpha=0.5, lw=3, label=False, ax=ax)
...
def plot_calendar_returns_info_graphic(daily_rets_ts, x_dim=15, y_dim=6):
    ...
    ann_ret_df = pd.DataFrame( timeseries.aggregate_returns(daily_rets_ts, 'yearly') )
...
def plot_avg_holdings(df_pos):
    ...
    df_holdings = df_pos.groupby([lambda x: x.year, lambda x: x.month]).apply(lambda x: np.mean([len(x[x!=0]) for _, x in x.iterrows()])).unstack()
...
def plot_holdings(df_pos, end_date=None):
    ...
    plt.axhline(df_holdings.values.mean(), color='steelblue', ls='--', lw=3, alpha=1.0)
...
def plot_drawdowns(df_rets, top=10):
    ...
    for i, (peak, recovery) in df_drawdowns[['peak date', 'recovery date']].iterrows():
        ax1.fill_between((peak, recovery), lim[0], lim[1], alpha=.5, color=colors[i])
