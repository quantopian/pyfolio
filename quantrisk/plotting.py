...
def plot_calendar_returns_info_graphic(daily_rets_ts, x_dim=15, y_dim=6):
    ...
    sns.heatmap(monthly_ret_table.fillna(0)*100.0, annot=True, annot_kws={"size": 12}, alpha=1.0, center=0.0, cbar=False, cmap=matplotlib.cm.RdYlGn)
    ax1.set_ylabel(' ')
...
def plot_scatter(x1Values, y1Values,
...
def plot_performance_pivot_table_heatmaps(results_df, x_axis_var, y_axis_var):
    ...
    results_df_sharpe = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='sharpe_ratio')
    results_df_max_drawdown = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='max_drawdown')
    results_df_annual_return = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='annual_return')
    results_df_alpha = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='alpha')
    results_df_beta = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='beta')
    results_df_volatility = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='annual_volatility')
    results_df_stability = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='stability')
    results_df_calmar = results_df.pivot_table(index=heatmap_y, columns=heatmap_x, values='calmar_ratio')

    fig = plt.figure(figsize=(17,19))

    ax1 = fig.add_subplot(3,3,1)
    ax1.set_title("Sharpe Ratio", fontsize=16)
    ax1 = sns.heatmap(results_df_sharpe, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Greens)

    ax2 = fig.add_subplot(3,3,2)
    ax2.set_title("Annual Return", fontsize=16)
    ax2 = sns.heatmap(results_df_annual_return, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Greens)

    ax3 = fig.add_subplot(3,3,3)
    ax3.set_title("Max Drawdown")
    ax3 = sns.heatmap(results_df_max_drawdown, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Reds_r)

    ax4 = fig.add_subplot(3,3,4)
    ax4.set_title("Volatility", fontsize=16)
    ax4 = sns.heatmap(results_df_volatility, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Reds)

    ax5 = fig.add_subplot(3,3,5)
    ax5.set_title("Beta", fontsize=16)
    ax5 = sns.heatmap(results_df_beta, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Blues)

    ax6 = fig.add_subplot(3,3,6,)
    ax6.set_title("Alpha", fontsize=16)
    ax6 = sns.heatmap(results_df_alpha, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Greens)

    ax7 = fig.add_subplot(3,3,7)
    ax7.set_title("Stability", fontsize=16)
    ax7 = sns.heatmap(results_df_stability, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Blues)

    ax7 = fig.add_subplot(3,3,8)
    ax7.set_title("Calmar Ratio", fontsize=16)
    ax7 = sns.heatmap(results_df_calmar, annot=True, annot_kws={"size": 14}, cmap=plt.cm.Greens)
...
def plot_rolling_risk_factors(my_algo_returns, risk_factors, rolling_beta_window=63, legend_loc='lower left'):
    plt.figure(figsize=(13,4))
    plt.title("Rolling Fama-French Single Factor Betas",fontsize=16)
    plt.ylabel('beta',fontsize=12)

    rolling_risk_multifactor = qts.rolling_multifactor_beta(my_algo_returns, risk_factors.ix[:,['SMB','HML','UMD']],
                                                            rolling_window=rolling_beta_window)

    rolling_beta_SMB = qts.rolling_beta(my_algo_returns, risk_factors['SMB'], rolling_window=rolling_beta_window)
    rolling_beta_HML = qts.rolling_beta(my_algo_returns, risk_factors['HML'], rolling_window=rolling_beta_window)
    rolling_beta_UMD = qts.rolling_beta(my_algo_returns, risk_factors['UMD'], rolling_window=rolling_beta_window)
    ...
    plt.legend(['Small-Caps (SMB)', 'High-Growth (HML)', 'Momentum (UMD)'], loc=legend_loc)

    plt.figure(figsize=(13,3))

    (rolling_risk_multifactor['const']*252).plot(color='forestgreen', alpha=0.5, lw=3, label=False)
    plt.axhline( (rolling_risk_multifactor['const']*252).mean(), color='darkgreen', alpha=0.8, lw=3, ls='--')
    plt.axhline( 0.0, color='black')
    plt.xlim( (my_algo_returns.index[0], my_algo_returns.index[-1]) )
    plt.ylabel('alpha', fontsize=12)
    plt.title('Multi-factor Alpha (vs. Factors: Small-Cap, High-Growth, Momentum)', fontsize=16)
