...
def string_arr_to_single_str(
    ...
    for i in range(len(strings_arr)):
        if i > 0:
            res_str = res_str + sep_char + \
                prefix_char + strings_arr[i] + suffix_char
        else:
...
def determine_next_filename(
    ...
    while file_version > 0:
        temp_filename = primary_filename + '_' + \
            str(file_version) + filename_ext
        # print(temp_filename)
...
def union_tuple_elements(arrayOfTuples):
    tempUnion = []
